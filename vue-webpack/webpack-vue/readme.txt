Webpack 是一个现代的 js 应用的静态模块化打包工具
    模块: commonjs/es6 规范
    打包: webpack 打包之后, 将代码处理成各种浏览器都能兼容处理的代码 -- 比如把 es6 代码转化为 es5
         处理模块之间的依赖关系
webpack 依赖于 node 环境
node 为了正常运行, 需要依赖各种包, 因此 node 自带 npm-包管理工具
...
2. 安装 webpack: webpack 依赖 node 环境 -- 因此必须先安装 nodejs, 然后通过 nodejs 自带的 npm 安装 webpack
    Webpack: 前端模块化打包工具
    全局安装: 也可以不指定版本, 默认最新
...
-------------
.babelrc: 配置 babel, 将 es6 语法转化为 es6, 保证浏览器的适配性
.editorconfig: 代码格式规范
